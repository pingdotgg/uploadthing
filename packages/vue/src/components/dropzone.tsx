    ...
    defineComponent(
        (props: { config: UploadDropzoneProps<TRouter, TEndpoint> }) => {
            ...
            const fileTypesText = computed(() =>
                contentFieldToContent($props.content?.allowedContent, styleFieldArg.value) ??
                    generatedPermittedFileTypes.value.fileTypes.length > 0
            ...
            const uploadIconContent = computed(() => contentFieldToContent($props.content?.uploadIcon, styleFieldArg.value) ?? (
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 20 20"
                    class={twMerge(
                        "mx-auto block h-12 w-12 align-middle text-gray-400",
                        styleFieldToClassName($props.appearance?.uploadIcon, styleFieldArg.value),
                    )}
            ...
            const containerClass = computed(() =>
                twMerge(
                    ...
                    $props.className,
                    styleFieldToClassName($props.appearance?.container, styleFieldArg.value),
                )
            );
            const labelClass = computed(() => twMerge(
                classNames(
            ...
            const allowedContentClass = computed(() => twMerge(
            ...
            const buttonClass = computed(() => twMerge(
                classNames(
                    ...
                    state.value === "uploading"
                        ? `bg-blue-400 after:absolute after:left-0 after:h-full after:bg-blue-600 ${progressHeights[uploadProgress.value]}`
                        : "bg-blue-600",
                ),
                styleFieldToClassName($props.appearance?.button, styleFieldArg.value),
            ))

            const containerStyle = computed(() => styleFieldToCssObject($props.appearance?.container, styleFieldArg.value))
            const labelStyle = computed(() => styleFieldToCssObject($props.appearance?.label, styleFieldArg.value))
            const allowedContentStyle = computed(() => styleFieldToCssObject(
                $props.appearance?.allowedContent,
                styleFieldArg.value,
            ))
            const buttonStyle = computed(() => styleFieldToCssObject(
                $props.appearance?.button,
                styleFieldArg.value,
            ))

            const labelContent = computed(() => contentFieldToContent($props.content?.label, styleFieldArg.value) ?? (ready.value
                ? `Choose files or drag and drop`
                : `Loading...`))
            const buttonContent = computed(() => contentFieldToContent($props.content?.button, styleFieldArg.value) ?? (
                isUploading.value ? (
                    <Spinner />
                ) : (
