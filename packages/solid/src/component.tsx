...
import { internalUploadthingHookGen } from './useUploadThing.ts';
...
const capitalizeStart = (str: string) => str.charAt(0).toUpperCase() + str.slice(1);

const internalDoFormatting = (config?: ExpandedRouteConfig): string => {
  if (!config) return '';
...
): string => capitalizeStart(internalDoFormatting(config));

const onUploadProgressExample = (progress: number): void | undefined => {
  if (progress) return undefined;
  return undefined;
};

const onClientUploadCompleteExample = (
  res?: Awaited<ReturnType<UploadFileType<FileRouter>>>,
): void | undefined => {
  if (res) return undefined;
  return undefined;
};

const onUploadErrorExample = (error: Error): void | undefined => {
...
export function UploadButton<TRouter extends FileRouter>(
  ...
  const useUploadThing = internalUploadthingHookGen<TRouter>();
  ...
  const fileInfo = () => generatePermittedFileTypes(uploadedThing.permittedFileInfo()?.config);
      ...
      >
        <input
          ...
          onChange={(e) => {
            ...
            uploadedThing.startUpload(files, input);
          }}
        />
...
export function UploadDropzone<TRouter extends FileRouter>(
  ...
  const uploadInfo = () => generatePermittedFileTypes(uploadedThing.permittedFileInfo()?.config);
