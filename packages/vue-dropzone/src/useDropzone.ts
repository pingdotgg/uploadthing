...
import { ReducerState, useReducer } from "./useReducer";
...
// it's used nowhere but leaving for reference
...
export function useDropzone(
  options: Ref<Partial<FileUploadOptions>> | (() => Partial<FileUploadOptions>),
) {
  ...
  async function onDragEnterCb(event: Event) {
    const { getFilesFromEvent, noDragEventsBubbling, onDragEnter } =
      optionsRef.value;
    event.preventDefault();
  ...
  function onDragLeaveCb(event: Event) {
    ...
    const targets = dragTargetsRef.value.filter((target: EventTarget) => {
      if (!rootRef.value) {
        return false;
      }
      const rootElm = rootRef.value.$el || rootRef.value;
  ...
  const composeHandler = (fn: ComposeFunction) =>
    optionsRef.value.disabled ? undefined : fn;

  const composeKeyboardHandler = (fn: ComposeFunction) =>
    optionsRef.value.noKeyboard ? undefined : composeHandler(fn);

  const composeDragHandler = (fn: ComposeFunction) =>
    optionsRef.value.noDrag ? undefined : composeHandler(fn);
  ...
  const getRootProps = ({
    ...
    onDragenter: composeDragHandler(
      composeEventHandlers(onDragEnter, onDragenter, onDragEnterCb),
    ),
    onDragover: composeDragHandler(
      composeEventHandlers(onDragOver, onDragover, onDragOverCb),
    ),
    onDragleave: composeDragHandler(
      composeEventHandlers(onDragLeave, onDragleave, onDragLeaveCb),
    ),
    onDrop: composeDragHandler(composeEventHandlers(onDrop, onDropCb)),
    ref: rootRef,
    ...(!optionsRef.value.disabled && !optionsRef.value.noKeyboard
      ? { tabIndex: 0 }
      : {}),
    ...rest,
  });
  ...
  const fileCount = computed(() =>
    state.draggedFiles ? state.draggedFiles.length : 0,
  );
  ...
  const isDragReject = computed(
    () => fileCount.value > 0 && !isDragAccept.value,
  );
