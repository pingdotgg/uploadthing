...
import type { ClientUploadFileResponse } from "uploadthing/types";
...
it("infers output properly", () => {
  doNotExecute(async () => {
    ...
    expectTypeOf<ClientUploadFileResponse<{ foo: "bar" }>[] | undefined>(res);
  });

  doNotExecute(async () => {
    const { startUpload } = useUploadThing("withFooInput");
    const res = await startUpload(files, { foo: "bar" });
    expectTypeOf<ClientUploadFileResponse<{ baz: "qux" }>[] | undefined>(res);
  });

  doNotExecute(async () => {
    const { startUpload } = useUploadThing("withBarInput");
    const res = await startUpload(files, { bar: 1 });
    expectTypeOf<ClientUploadFileResponse<null>[] | undefined>(res);
  });
