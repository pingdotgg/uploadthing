...
FROM THE RIGHT PLACE. The export name ensures your file routes' [`middleware`](/api-reference/file-routes#middleware)
...
A helper class to construct [`File`](https://developer.mozilla.org/en-US/docs/Web/API/File) in environments that don't support it natively.

Also accepts a `customId` property to set a custom identifier for the file to be uploaded using [UTApi.uploadFiles](/api-reference/ut-api#upload-files).
