...
The first argument are the URLs of the files you want to upload. They may also be an object with a `url` property in case you
want to override the `name`, or set a `customId` for the files. The function also takes some optional options:
...
`listFiles` returns a paginated list of objects containing file ids and keys for all
files that have been uploaded to the application your API key corresponds to.
...
Retrieve a [signed URL](/concepts/regions-acl#access-controls) for a private file.
...
Update the [ACL](/concepts/regions-acl#access-controls) of a file or a list of files.
