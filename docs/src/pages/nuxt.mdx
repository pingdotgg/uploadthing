...
The `@uploadthing/nuxt` package includes "useUploader" builder that you can use
to generate type-safe component, drop it into app and start uploading files.
...
<template>
  ...
  >
    ...
    => { alert("Upload Completed"); }} onUploadError={(error: Error) => { // Do
    something with the error. alert(`ERROR! ${error.message}`); }} }" />
  </main>
</template>
```
...
<template>
  <main
    class="flex min-h-screen flex-col items-center justify-center gap-16 p-24"
  >
    <UploadButton :config="{ endpoint: 'videoAndImage',
    onClientUploadComplete={() => { alert("Upload Completed"); }}
    onUploadError={(error: Error) => { // Do something with the error.
...
<template>
  <main
    class="flex min-h-screen flex-col items-center justify-center gap-16 p-24"
  >
    <UploadDropzone :config="{ endpoint: 'videoAndImage',
    onClientUploadComplete={() => { alert("Upload Completed"); }}
    onUploadError={(error: Error) => { // Do something with the error.
    alert(`ERROR! ${error.message}`); }} }" />
  </main>
