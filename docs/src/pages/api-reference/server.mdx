...
`uploadFiles` let's you upload files directly from your server, without having
to define file routes. When uploadthing files from the server, no presigned URLs
are used, your files are uploaded directly to S3 via our servers.

This can be useful for uploading files from sever actions, or your own API
routes.
...
`uploadFileFromUrl` is similar to `uploadFiles` as it let's you upload files
directly from your server, without having to define file routes.

When uploading files from URL, the file is first downloaded on **your** server,
before being uploaded to S3 via our servers.
...
async function uploadFile(formData: FormData) {
  ...
  const response = await utapi.uploadFileFromUrl(url);
  //    ^? { key: string, url: string }
}
...
| Arguments | Type            | Required | Description                            |
| :-------- | :-------------- | :------- | :------------------------------------- |
| url       | URL \|\| string | Yes      | The URL of the file you wish to upload |
| metadata  | Json            | No       | Metadata to attach to the file(s)      |
