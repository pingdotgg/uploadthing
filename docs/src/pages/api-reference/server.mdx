...
<Callout>
  ACL can only be overridden if set on the UploadThing dashboard. Attempts to
  upload a mismatching ACL setting will error.
</Callout>
...
| contentDisposition | "inline" \| "attachment"   | No       | What [content disposition](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition) should be set on the storage provider. Defaults to `inline` |
...
<Callout>
  The `expiresInSeconds` option can only be used if you allow overrides in your
  app settings on the UploadThing dashboard.
</Callout>

| Arguments                | Type   | Required | Description                                                                           |
| :----------------------- | :----- | :------- | :------------------------------------------------------------------------------------ |
| key                      | string | Yes      | The file key you want to get the URL for.                                             |
| options.expiresInSeconds | number | No       | How long the URL should be valid for. Must be positive and less than 604800 (7 days). |
