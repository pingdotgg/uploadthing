...
| onChange               | `(files: File[]) => void`                            | No                      | Added in `v6.8` | Callback function that runs when files are selected. Note: `files` is an array containing _all_ selected files, not just the latest ones. |
...
export type UploadFileResponse<TServerOutput> = {
  ...
  size: number;
  key: string;
  url: string; // e.g. `https://utfs.io/f/abc-123.txt`
  appUrl: string; // e.g. `https://utfs.io/a/app-1/abc-123.txt`
...
For the difference of the two URLs, see
[Accessing Files](/working-with-files#accessing-files).
...
| onChange               | (acceptedFiles: File[]) => void                      | No                      | Added in `v6.8` (`onDrop` in `v6.5-v-6.7`) | Callback called when files are dropped or pasted.                                                                           |
