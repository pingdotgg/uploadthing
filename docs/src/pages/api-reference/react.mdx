...
| onSelect               | `(files: File[]) => void`                            | No                      | Added in `v6.8` | Callback function that runs when files are selected. Note: `files` is an array containing _all_ selected files, not just the latest ones. |
