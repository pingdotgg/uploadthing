...
[`.middleware()`](/api-reference/server#middleware) function in each file route.
This makes it trivial to protect some file routes, and keep some public.
...
```ts filename="~/server/uploadthing"
...
<Callout type="info">
  ...
  [`errorFormatter`](/errors#error-formatting) to control what is sent down to
  the client.
</Callout>
