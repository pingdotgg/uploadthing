...
transferring the files through your server. Your server instead acts as a proxy
for requesting presigned URLs from the UploadThing API that the client will then
use to upload the files directly to the storage provider.
...
<Callout type="default">
  ...
  using [File Routes](/api-reference/server#file-routes) that let you define
...
[File Route](/api-reference/server#file-routes), expose it on your server using
one of our adapters, and then upload the files using our built-in components or
...
need to request the presigned URLs from the UploadThing API. Include the file
information, the result of running the
[file route middleware](/api-reference/server#middleware) as well as the
...
The response will contain an array of objects, one for each file. Depending on
the size of the file, it's corresponding object will contain either a string
`url` or a array of strings `urls`. The `urls` list will be used for multi-part
uploads while a single URL string should be submitted as a single part upload.
Return them back to the client.

### Uploading the files using the presigned URLs

For each object in the array you received in the previous step, check if it
contains the `url` or `urls` field, and upload accordingly:

#### Single Part Upload

Submit a POST request to `presigned.url` you received in the previous step.
...
<Callout type="warning">
  It is important that the file is the last field in the `FormData` object,
  otherwise the storage provider may reject the upload.
</Callout>
...
Once all the parts have been uploaded, you will need to tell Uploadthing that
...
If some parts did not upload successfully, you can implement retry logic to
attempt the upload of those parts again. If the retries still fail, notify
Uploadthing of the upload failure to release the reserved storage from your
application's quota.:
...
If you want to get data returned from the server-side `onUploadComplete`
callback, you can poll for it using the `/pollUpload` endpoint. Include the
`pollingJwt` returned alongside the presigned URL to authenticate the request.
...
ensure the request is authentic and originates from the UploadThing API.
...
<Callout type="info">
  Please note that waiting for the server callback data requires additional
  roundtrips which take time and can lead to a decreased user experience. If you
  do not need the server callback data, don't wait for it on the client side. If
  you're using our SDKs, you can skip polling by setting the `skipPolling`
  option to `true`.
</Callout>
...
similar to client side uploads.

### Requesting presigned URLs

Requesting presigned URLs works the same way as for client side uploads. You'll
need to send a request to `/prepareUpload` and include the information about the
file, what [access controls](/regions-and-acl#access-controls) it should have as
well as the
