...
app's region on the UploadThing dashboard. If no configuration is done, your
files are uploaded to our default region which is currently `AWS us-west-2`.

<Callout type="info">
  Uploading to a region different than the default one requires a paid plan.
</Callout>

### Configuring region

Go to your app' settings on the UploadThing dashboard. Under `Regions and ACL`,
...
<Callout type="info">
  ...
  region, it will only affect new files.
</Callout>

## Access Controls

By default every file uploaded to UploadThing is accessible simply by it's URL
(`utfs.io/f/FILE_KEY`). Although this is secure enough for many applications
since the file key is a very hard-to-guess random string, some applications
require a more secure way to store their files.
...
- `private`: Private files will not be accessible simply by it's URL, but
  requires a short-lived signed URL to be accessed.
...
<Callout type="info">
  Private files are only available on paid plans and regions other than the
  default one.
</Callout>
