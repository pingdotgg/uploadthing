...
#### Not using TypeScript V4.9 or above?

Not using `v4.9` or above, meaning you won't get access to the `satisfies`
keyword, therefore you should be using `f.router` to define a router.
...
export const uploadRouter = f.router({
  withoutMdwr: f({
    image: {
      maxFileCount: 2,
      maxFileSize: "16MB",
